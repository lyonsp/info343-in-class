...
describe("redux store", function() {
    ...
    it("should add a favorite", function() {
        ...
        // equal tests if expression is literally equal (triple equal)
        // deep equal can be used with objects to check if properties the same
        // not triple equal, but have same keys and values
        expect(state2.favorites[0]).to.deep.equal(item);
        expect(state1).to.not.equal(state2);
    });

    it("remove a favorite", function() {
        store.dispatch(removeFavorite(item.id));
        var state = store.getState();
        expect(state.favorites.length).to.equal(0);
    });

    it("should ignore and unknown action", function() {
        var state1 = store.getState();
